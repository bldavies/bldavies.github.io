...
I recently discovered R's [`igraph`](http://igraph.org) package for analysing networks and have been using it to investigate the social behaviour of wild Pokémon species as suggested by their habitat choices.
This post documents and summarises some of my findings.
...
Using these data, I generate a species-location incidence matrix `habits` via
...
### Comparing ubiquity across types

The rows and columns of `habits` provide count data on where different species habitate.
For example, summing the rows of `habits` yields the *ubiquity* of each species, which I define as the number of locations in which each species habitates.
...
The three most ubiquitous species are Goldeen, Magikarp and Poliwag, each of which habitate in 24 distinct locations across the Kanto region.
...
### Comparing ubiquity and average diversity across species

The *diversity* of each location, which I define as the number of different species that choose the location as a habitat, is obtained by summing the columns of `habits`.
I store these sums in a corresponding `locations` data frame:

```r
locations <- data.frame(name = colnames(habits), diversity = colSums(habits))
```

I compute the average diversity of the locations in which each species habitates via
...
However, inspecting the data reveals that this negative relationship is skewed by a large number of species that cohabitate in one or two locations.
The chart below plots species' average diversity against their ubiquity, along with the line that best fits the data in a least-squares sense.
...
Outside of the top-left cluster, average diversity and ubiquity appear to vary independently, suggesting that species' social preferences are more complex than can be characterised by the two categories mentioned above.

## The cohabitation network

I probe species' social behaviour further by analysing their tendency to cohabitate.
I assume that species reveal their preference toward socialising with other species through their choice of whether to share habitats; the more frequently two species cohabitate, the stronger is their social connection.
...
I interpret `cohabits` as the adjacency matrix for a weighted graph on the set of wild Pokémon species in which 
...
A convenient measure of the tendency for two species to spend most of their time in each others' company is the [Jaccard index](https://en.wikipedia.org/wiki/Jaccard_index), which I use to count the number of locations in which two species cohabitate as a proportion of the locations in which at least one habitates.
...
If `C = cohabits` then each column of the matrix `U` is equal to the vector `pokemon$ubiquity` of species' ubiquity, and each entry `H[i, j]` of the divisor matrix `H` counts the number of locations in which at least one of species `i` and `j` habitate.
...
The more that two species make similar habitat choices, the higher is their shared Jaccard index.
...
These arise from the fact that the diagonal entries of `jaccard(cohabits)` are each equal to unity.
...
Accordingly, I augment the MSF by taking its union with the subgraph obtained from choosing the edges in `net` of highest weight.
...
A simple measure of such influence is the [degree centrality](https://en.wikipedia.org/wiki/Centrality#Degree_centrality) of each species; that is, the number of other species with which each species cohabitates.
...
The three most degree-central species are, unsurprisingly, also the three most ubiquitous, and each cohabitates with 82 of the 99 other species in the sample.
...
More betweenness-central species tend to have more control over the spread of information due to their relative criticality within other species' communication channels.
